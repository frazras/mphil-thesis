...
always easy to accomplish for novice developers. This project will outline
...
This would further assist in the commoditisation of robotics kits and subsequently
...
and automation, the \texttt{X10ABOT} robotics platform would be an attractive tool to be utilized in their projects.
...
We reviewed many types of robotics projects and came across a few interesting concepts. They influenced and validated the design decisions made in the \xten project. We will review a few of them and present an overview of their architectures. 
...
A comparable system that is currently used many in schools and by hobbyist is the Lego Mindstors robotics kit. The controller for the mindstroms kit without sensors and actuators starts at \$250. With a proper sheild built on an Arduino running the \xten software, a comparable kit with respect to the number of ports for sensors and actuators would cost no more than \$30 USD. The Midstorms kit is a polished product with a lot of additional features that far surpass the \xten, however the \xten has far better scalability, modularity and extensibility at a significantly lower price point. These features are not easily attained with a simple Arduino kit, the framework transforms the lowly Arduino into a capable robotics platform.
